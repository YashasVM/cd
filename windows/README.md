...
To add a per-user Explorer **Send to Phone** entry, pass the published
executable explicitly (no administrator rights required):

```powershell
.\windows\Install-SendToPhone.ps1 -Mode Install -ExePath .\windows\artifacts\win-x64\CD.exe
.\windows\Install-SendToPhone.ps1 -Mode Remove
```

Selecting files or folders in Explorer and choosing **Send to Phone** opens CD
with them preloaded in Send Files. It does not start a transfer automatically.
