...
func TestPlainRemovesTerminalStyles(t *testing.T) {
	styled := Filename("transfer.txt", true) + " " + Success("done", true)
	if got := Plain(styled); got != "transfer.txt done" {
		t.Fatalf("plain text = %q; want %q", got, "transfer.txt done")
	}
}
...
func TestSemanticStylesUseGitLikePalette(t *testing.T) {
	tests := []struct {
		...
		{name: "filename", got: Filename("cdx", true), style: Bold},
