// Package store implements cdx's anonymous, encrypted, download-limited
...
func (s *Service) create(response http.ResponseWriter, request *http.Request) {
	...
	releaseActive = false
	response.Header().Set("X-Cdx-Downloads", strconv.Itoa(meta.DownloadsTotal))
...
func (s *Service) uploadObject(response http.ResponseWriter, request *http.Request, id string, index int) {
	...
	digest := strings.TrimSpace(request.Header.Get("X-Cdx-SHA256"))
...
func remainingDownloadsHeader(response http.ResponseWriter, remaining int) {
	response.Header().Set("X-Cdx-Downloads-Remaining", strconv.Itoa(remaining))
}
...
func (s *Service) downloadManifest(response http.ResponseWriter, request *http.Request, id string) {
	...
	response.Header().Set("X-Cdx-Expires-At", expires.UTC().Format(time.RFC3339))
