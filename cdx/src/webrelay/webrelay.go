// Package webrelay serves the embedded cdx web client and exposes its
// deliberately small WebSocket-to-TCP bridge. The bridge never participates
// in the cdx protocol; it only forwards an opaque byte stream to one fixed
...
// Config configures the unified cdx web server.
type Config struct {
...
// Handler returns the unified cdx web handler. It serves the embedded client,
...
func Run(ctx context.Context, config Config) error {
	...
	go func() {
		log.Infof(
			"starting cdx web server on %s for %s via %s (%s)",
...
func (h *handler) config(w http.ResponseWriter, r *http.Request) {
	...
	_, _ = fmt.Fprintf(w, "window.__CDX_RUNTIME_CONFIG__ = %s;\n", payload)
}
...
func (h *staticHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	if r.URL.Path == "/__cdx_download__" ||
		strings.HasPrefix(r.URL.Path, "/__cdx_download__/") {
	...
	if page, exists := h.htmlPages[requested]; exists {
		...
		return
	} else if requested == "cdx-download-sw.js" || requested == "cdx-worker.js" {
		w.Header().Set("Cache-Control", "no-cache")
