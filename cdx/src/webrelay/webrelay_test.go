...
func testSite() fstest.MapFS {
	return fstest.MapFS{
		...
		"assets/app.js":  {Data: []byte("console.log('cdx')")},
...
func TestServesSiteAndRuntimeConfig(t *testing.T) {
	...
	handler.ServeHTTP(
		recorder,
		httptest.NewRequest(http.MethodGet, "/cdx-download-sw.js", nil),
	)
	assert.Equal(t, http.StatusOK, recorder.Code)
	assert.Equal(t, "no-cache", recorder.Header().Get("Cache-Control"))
	...
	for _, method := range []string{http.MethodGet, http.MethodHead} {
		...
		handler.ServeHTTP(
			recorder,
			httptest.NewRequest(method, "/__cdx_download__/test-id", nil),
		)
		assert.Equal(t, http.StatusNotFound, recorder.Code)
	...
	assert.JSONEq(
		...
		strings.TrimSuffix(
			strings.TrimPrefix(recorder.Body.String(), "window.__CDX_RUNTIME_CONFIG__ = "),
			";\n",
		),
	)
}
...
func TestWebSocketForwardsBinaryStream(t *testing.T) {
	...
	payload := []byte("cdx\x05\x00\x00\x00hello")
