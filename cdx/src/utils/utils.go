...
func GetConfigDir(requireValidPath bool) (homedir string, err error) {
	if envHomedir, isSet := os.LookupEnv("CDX_CONFIG_DIR"); isSet {
		homedir = envHomedir
	} else if xdgConfigHome, isSet := os.LookupEnv("XDG_CONFIG_HOME"); isSet {
		homedir = path.Join(xdgConfigHome, "cdx")
	} else {
		...
		homedir = path.Join(homedir, ".config", "cdx")
	}
...
// GetRandomName returns a random three-word cdx code.
//
// It retains its historical signature for callers outside transfer. New code that
...
func ZipDirectoryWithExactExclusions(destination string, source string, ignoredPaths map[string]bool, exclusions, exactExclusions []string) (err error) {
	...
	// no compression because cdx does its compression on the fly
...
// limited to the archive's on-disk size. Cdx-created zip archives use deflate
...
func unzipDirectoryFromReaderWithLimit(
	...
	staging, err := root.CreateTempDir(".", ".cdx-extract-")
	if err != nil {
...
func MarkFileForRemoval(fname string) {
	// Cleanup is only for temporary files cdx creates in its working directory.
