...
func TestRemoveMarkedFilesIgnoresTransferredCleanupFile(t *testing.T) {
	...
	transferredCleanupFile := filepath.Join(receiverDir, "cdx-marked-files.txt")
...
func TestRemoveMarkedFilesRemovesOnlyLocalMarkedFiles(t *testing.T) {
	...
	localTarget := filepath.Join(workingDir, "cdx-temp-file")
...
func TestResolveUnzipPathRejectsNonLocalEntries(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:  "absolute path",
			entry: filepath.Join(string(os.PathSeparator), "tmp", "cdx-absolute-escape.txt"),
		},
		{
		...
		},
		{
			...
			entry: filepath.Join("..", "cdx-relative-escape.txt"),
		},
		{
			name:  "nested parent traversal",
			entry: filepath.Join("safe", "..", "..", "cdx-relative-escape.txt"),
		},
		{
			name:  "empty path",
			entry: "",
...
// Regression test for https://github.com/schollz/cdx/issues/1087.
