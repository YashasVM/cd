// Package codephrase generates cdx codes and resolves them into relay-room
// and PAKE components.
package codephrase
...
// Format identifies how a cdx code is divided into its room and PAKE parts.
type Format string
...
// ErrCodeTooShort is returned when a cdx code is shorter than six bytes.
...
// Components contains the protocol inputs derived from a user-facing cdx
...
func generate(reader io.Reader) (string, error) {
	...
	for i := range words {
		...
		if err != nil {
			return "", fmt.Errorf("generate cdx code word %d: %w", i+1, err)
		}
...
// Parse resolves a cdx code into the relay room and PAKE passphrase used by
...
// four-word format; every other valid code uses cdx's legacy byte-based split.
...
// the word "yo-yo", which uses the same hyphen as cdx's word separator.
