// Package webcli implements the standalone cdx-web server command.
package webcli
...
// Version specifies the version reported by cdx-web.
...
func newApp(ctx context.Context) *cli.App {
	...
	app.Usage = "serve the cdx website and browser relay"
	app.UsageText = "cdx-web [OPTIONS] [public-host[:port]]"
	app.ArgsUsage = "[public-host[:port]]"
	app.Flags = []cli.Flag{
		&cli.StringFlag{Name: "bind", Value: "127.0.0.1:9014", Usage: "local HTTP bind address"},
		&cli.StringFlag{Name: "relays", Aliases: []string{"relay"}, Value: strings.Join(publicrelay.Relays(), ","), Usage: "ordered comma-separated upstream cdx relay hosts"},
		...
		&cli.IntFlag{Name: "store-downloads", Value: 1, Usage: "maximum verified downloads per stored transfer", EnvVars: []string{"CDX_STORE_DOWNLOADS"}},
		&cli.StringFlag{Name: "store-max-expiration", Value: "0", Usage: "maximum stored lifetime (m, h, d, or w; 0 is unlimited)", EnvVars: []string{"CDX_STORE_MAX_EXPIRATION"}},
...
func serve(ctx context.Context, c *cli.Context) error {
	if c.Args().Len() > 1 {
		return errors.New("cdx-web accepts one public website address")
	}
	if c.Bool("debug") {
		log.SetLevel("debug")
