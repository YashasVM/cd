...
func TestAppIdentityAndArguments(t *testing.T) {
	...
	if app.Name != "cdx-web" {
		t.Fatalf("app name = %q, want cdx-web", app.Name)
	}
	if err := app.Run([]string{"cdx-web", "one.example", "two.example"}); err == nil {
...
func TestStoreDownloadsFlagParsing(t *testing.T) {
	for _, testCase := range []struct {
		...
		{name: "default", args: []string{"cdx-web"}, want: 1},
		{name: "custom", args: []string{"cdx-web", "--store-downloads", "6"}, want: 6},
	} {
		t.Run(testCase.name, func(t *testing.T) {
			t.Setenv("CDX_STORE_DOWNLOADS", "")
			app := newApp(context.Background())
			var got int
...
func TestStoreDownloadsEnvironmentConfiguration(t *testing.T) {
	t.Setenv("CDX_STORE_DOWNLOADS", "12")
	app := newApp(context.Background())
	var got int
	app.Action = func(ctx *internalcli.Context) error {
		got = ctx.Int("store-downloads")
	...
	if err := app.Run([]string{"cdx-web"}); err != nil {
...
func TestStoreMaxExpirationConfiguration(t *testing.T) {
	t.Run("default is unlimited", func(t *testing.T) {
		t.Setenv("CDX_STORE_MAX_EXPIRATION", "")
		if err := os.Unsetenv("CDX_STORE_MAX_EXPIRATION"); err != nil {
			t.Fatal(err)
		}
		app := newApp(context.Background())
		var got string
	...
	t.Run("environment", func(t *testing.T) {
		t.Setenv("CDX_STORE_MAX_EXPIRATION", "2w")
		app := newApp(context.Background())
		var got string
		app.Action = func(ctx *internalcli.Context) error {
			got = ctx.String("store-max-expiration")
	...
	t.Run("flag overrides environment", func(t *testing.T) {
		...
		if err := app.Run([]string{"cdx-web", "--store-max-expiration", "3d"}); err != nil {
			t.Fatal(err)
		}
		if got != "3d" {
