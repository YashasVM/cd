...
func TestInstallerDeterminesLatestVersion(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			tool: "curl",
			response: `{
  "url": "https://api.github.com/repos/schollz/cdx/releases/1",
  "tag_name": "v11.2.3",
  "draft": false,
  "prerelease": false
}`,
...
func TestInstallerBuildsReleaseAssetURLsFromDynamicVersion(t *testing.T) {
	...
	for _, fragment := range []string{
		`cdx_file="${cdx_bin_name}_v${cdx_version}_${cdx_os}-${cdx_arch}.${cdx_dl_ext}"`,
		`cdx_checksum_file="${cdx_bin_name}_v${cdx_version}_checksums.txt"`,
		`cdx_url="${cdx_base_url}/v${cdx_version}/${cdx_file}"`,
		`cdx_checksum_url="${cdx_base_url}/v${cdx_version}/${cdx_checksum_file}"`,
	} {
		if !strings.Contains(script, fragment) {
			t.Fatalf("installer does not contain URL construction %q", fragment)
		}
	}
	if regexp.MustCompile(`cdx_version="[0-9]+\.[0-9]+\.[0-9]+"`).MatchString(script) {
		t.Fatal("installer still contains a hardcoded cdx version")
	}
}
