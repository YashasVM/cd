...
func TestWebReceiveURL(t *testing.T) {
	t.Setenv("CDX_WEB_URL", "")
	assert.Equal(t, "", webReceiveURL("1234-word/word?&"))

	t.Setenv("CDX_WEB_URL", "https://web.example.com/")
	assert.Equal(
		t,
		"https://web.example.com/?code=1234-word%2Fword%3F%26",
...
func TestRejectsUnsupportedPeerPakeVersion(t *testing.T) {
	...
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			if !strings.Contains(versionErr.Error(), "upgrade both cdx clients") {
...
func TestCdxReadme(t *testing.T) {
...
func TestCdxNonASCIIFileName(t *testing.T) {
	testDir := t.TempDir()
...
func TestCdxRenameExistingFile(t *testing.T) {
	testDir := t.TempDir()
	sourceDir := filepath.Join(testDir, "source")
	receiveDir := filepath.Join(testDir, "receive")
	if err := os.MkdirAll(sourceDir, 0o755); err != nil {
...
func TestCdxEmptyFolder(t *testing.T) {
	pathName := "../../testEmpty"
	...
	sender, err := New(Options{
		IsSender:      true,
		SharedSecret:  "8123-testingthecdx",
		Debug:         true,
		RelayAddress:  "127.0.0.1:8281",
		RelayPorts:    []string{"8281"},
		RelayPassword: "pass123",
...
func TestCdxSymlink(t *testing.T) {
	...
	sender, err := New(Options{
		IsSender:      true,
		SharedSecret:  "8124-testingthecdx",
		Debug:         true,
		RelayAddress:  "127.0.0.1:8281",
		RelayPorts:    []string{"8281"},
		RelayPassword: "pass123",
...
func TestCdxIgnoreGit(t *testing.T) {
	log.SetLevel("trace")
	...
	filesInfo, _, _, errGet := GetFilesInfo([]string{"../../LICENSE", ".gitignore", "transfer.go"}, false, true, []string{})
	if errGet != nil {
		t.Errorf("failed to get minimal info: %v", errGet)
	}
...
// https://github.com/schollz/cdx/issues/1087: --exclude and --git were
...
func TestCdxError(t *testing.T) {
	...
	sender, _ := New(Options{
		IsSender:      true,
		SharedSecret:  "8123-testingthecdx2",
		Debug:         true,
...
func TestReceiverStdoutWithInvalidSecret(t *testing.T) {
	// Test for issue: panic when receiving with --stdout and invalid CDX_SECRET
...
func TestValidChunkBounds(t *testing.T) {
	if !validChunkBounds(0, 4, 4) || !validChunkBounds(2, 2, 4) {
		t.Fatal("expected in-range chunks to be accepted")
	}
	for _, tc := range [][3]int64{{-1, 1, 4}, {4, 1, 4}, {3, 2, 4}} {
		if validChunkBounds(tc[0], tc[1], tc[2]) {
			t.Fatalf("accepted out-of-range chunk: position=%d length=%d size=%d", tc[0], tc[1], tc[2])
		}
	}
}
