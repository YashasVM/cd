...
func TestQuotedFilename(t *testing.T) {
	if got := quotedFilename("cdx.txt", false); got != "'cdx.txt'" {
		t.Fatalf("plain filename = %q", got)
	}
	if got := quotedFilename("cdx.txt", true); got != "'\x1b[1mcdx.txt\x1b[0m'" {
