...
func TestEmbeddedClientContainsEntryPointAndWasm(t *testing.T) {
	...
	for _, fragment := range [][]byte{
		...
		[]byte(`"ratingCount": 50`),
		[]byte(`"reviewCount": 50`),
		[]byte(`"reviewBody": "I use cdx here a lot. Awesome binary for me"`),
	} {
	...
	for _, fragment := range [][]byte{
		[]byte(`<title>PAKE, step by step | cdx field notes</title>`),
	...
	wasm, err := fs.Stat(files, "transfer.wasm")
	if err != nil {
	...
	for _, fragment := range [][]byte{
		...
		[]byte("https://api.github.com/repos/schollz/cdx/releases/latest"),
	...
	if regexp.MustCompile(`cdx_version="[0-9]+\.[0-9]+\.[0-9]+"`).Match(installer) {
		t.Fatal("embedded installer contains a hardcoded cdx version")
	}
}
