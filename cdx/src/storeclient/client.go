// Package storeclient implements the client side of cdx-store-v1 for the CLI.
package storeclient
...
// Client talks to one cdx stored-transfer HTTP service.
type Client struct {
	HTTP *http.Client
}
...
func (c *Client) createUploadWithOptions(
	...
	acceptedDownloads := 1
	if header := strings.TrimSpace(response.Header.Get("X-Cdx-Downloads")); header != "" {
...
func (c *Client) putWithRetry(ctx context.Context, target, token string, payload []byte) error {
	...
	for attempt := 0; attempt < 3; attempt++ {
		...
		request.Header.Set("X-Cdx-SHA256", storecrypto.EncodedSHA256(payload))
...
func (c *Client) Inspect(ctx context.Context, share storecrypto.Share) (storecrypto.Manifest, time.Time, error) {
	...
	expires, _ := time.Parse(time.RFC3339, response.Header.Get("X-Cdx-Expires-At"))
...
func (c *Client) startDownload(
	...
	statePath := ".cdx-store-" + share.ID + ".json"
...
func (c *Client) receiveFile(
	...
	partPath := path.Join(
		".",
		"."+file.Name+".cdx-"+session.share.ID+".part",
	)
...
func (c *Client) commit(ctx context.Context, share storecrypto.Share, claimToken string) (int, error) {
	...
	defer response.Body.Close()
	header := strings.TrimSpace(response.Header.Get("X-Cdx-Downloads-Remaining"))
	if header == "" {
