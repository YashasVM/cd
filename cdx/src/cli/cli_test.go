...
func TestRelayMaxRoomsOpenConfiguration(t *testing.T) {
	t.Run("default", func(t *testing.T) {
		unsetEnv(t, "CDX_MAX_ROOMS_OPEN")
		got := runRelayWithCapturedMaxRooms(t, []string{"cdx", "relay"})
	...
	t.Run("flag", func(t *testing.T) {
		unsetEnv(t, "CDX_MAX_ROOMS_OPEN")
		got := runRelayWithCapturedMaxRooms(t, []string{"cdx", "relay", "--max-rooms-open", "17"})
		if got != 17 {
	...
	t.Run("environment", func(t *testing.T) {
		t.Setenv("CDX_MAX_ROOMS_OPEN", "23")
		got := runRelayWithCapturedMaxRooms(t, []string{"cdx", "relay"})
		if got != 23 {
...
func TestRelayRejectsNonPositiveMaxRoomsOpen(t *testing.T) {
	for _, value := range []string{"0", "-1"} {
		t.Run(value, func(t *testing.T) {
			unsetEnv(t, "CDX_MAX_ROOMS_OPEN")
			app := newApp()
			err := app.Run([]string{"cdx", "relay", "--max-rooms-open=" + value})
			if err == nil {
...
func TestRelayHandshakeGuardConfiguration(t *testing.T) {
	t.Run("defaults", func(t *testing.T) {
		unsetEnv(t, "CDX_MAX_PENDING_HANDSHAKES")
		unsetEnv(t, "CDX_HANDSHAKE_TIMEOUT")
		got := runRelayWithCapturedConfig(t, []string{"cdx", "relay"})
	...
	t.Run("environment", func(t *testing.T) {
		t.Setenv("CDX_MAX_PENDING_HANDSHAKES", "23")
		t.Setenv("CDX_HANDSHAKE_TIMEOUT", "8m")
		got := runRelayWithCapturedConfig(t, []string{"cdx", "relay"})
...
func TestRelayRejectsNonPositiveHandshakeGuards(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			unsetEnv(t, "CDX_MAX_PENDING_HANDSHAKES")
			unsetEnv(t, "CDX_HANDSHAKE_TIMEOUT")
			err := newApp().Run([]string{"cdx", "relay", tt.arg})
			if err == nil {
...
func TestRelayAdmissionLimitConfiguration(t *testing.T) {
	for _, key := range []string{"CDX_SOURCE_JOIN_LIMIT", "CDX_ROOM_JOIN_LIMIT", "CDX_JOIN_LIMIT_WINDOW"} {
		unsetEnv(t, key)
	}
	t.Run("defaults", func(t *testing.T) {
		got := runRelayWithCapturedConfig(t, []string{"cdx", "relay"})
	...
	t.Run("environment", func(t *testing.T) {
		t.Setenv("CDX_SOURCE_JOIN_LIMIT", "13")
		t.Setenv("CDX_ROOM_JOIN_LIMIT", "4")
		t.Setenv("CDX_JOIN_LIMIT_WINDOW", "30s")
		got := runRelayWithCapturedConfig(t, []string{"cdx", "relay"})
...
// determinePass should trim a pass from --pass/CDX_PASS, not just from a file.
...
func TestRevokeIsRootFlag(t *testing.T) {
	...
	if err := app.Run([]string{"cdx", "--revoke", "transfer-id"}); err != nil {
...
func TestStoreDownloadsFlagParsing(t *testing.T) {
	for _, testCase := range []struct {
		...
		{name: "default", args: []string{"cdx", "send"}, want: 1},
		{name: "custom", args: []string{"cdx", "send", "--store-downloads", "4"}, want: 4},
	} {
...
func TestStoreExpirationFlagParsing(t *testing.T) {
	for _, testCase := range []struct {
		...
		{name: "default", args: []string{"cdx", "send"}, want: "1d"},
		{name: "custom", args: []string{"cdx", "send", "--store-expiration", "3d"}, want: "3d"},
	} {
		t.Run(testCase.name, func(t *testing.T) {
			app := newApp()
			for _, command := range app.Commands {
...
func TestServeIsRegistered(t *testing.T) {
	...
	if !found {
		t.Fatal("cdx should expose the built-in browser receive server as 'serve'")
	}
}
...
func TestUsesPublicRelayOnlyForUnmodifiedDefaults(t *testing.T) {
	tests := []struct {
		name       string
		args       []string
		mutate     func(*transfer.Options)
		wantPublic bool
	}{
		{name: "defaults", args: []string{"cdx", "send"}, wantPublic: true},
		{name: "explicit relay", args: []string{"cdx", "--relay", "relay.example:9009", "send"}},
		{name: "explicit relay6", args: []string{"cdx", "--relay6", "[::1]:9009", "send"}},
		{name: "local only", args: []string{"cdx", "--local", "send"}},
		{
			name: "remembered custom relay",
			args: []string{"cdx", "send"},
			mutate: func(options *transfer.Options) {
...
func TestAssignPublicRelayForCode(t *testing.T) {
	...
	for code, address := range tests {
		options := transfer.Options{SharedSecret: code, RelayAddress6: models.DEFAULT_RELAY6}
...
func TestBestPublicRelayCacheUsesAddressAndCurrentPoolOrder(t *testing.T) {
	t.Setenv("CDX_CONFIG_DIR", t.TempDir())
...
func TestSelectPublicRelayReplacesInvalidCache(t *testing.T) {
	for _, cached := range []string{"", "unknown:9009", " 2.getcroc.com:9009"} {
		t.Run(fmt.Sprintf("cached_%q", cached), func(t *testing.T) {
			configDir := t.TempDir()
			t.Setenv("CDX_CONFIG_DIR", configDir)
...
func TestSelectPublicRelayIgnoresCacheWriteFailure(t *testing.T) {
	...
	t.Setenv("CDX_CONFIG_DIR", configPath)
...
func TestRelayConnectionErrorClearsOnlyGeneratedPublicCache(t *testing.T) {
	tests := []struct {
		...
		{name: "relay failure", generated: true, err: fmt.Errorf("send: %w", transfer.ErrRelayConnection)},
		{name: "peer failure", generated: true, err: errors.New("peer disconnected"), wantCache: true},
		{name: "custom code", err: transfer.ErrRelayConnection, wantCache: true},
	}
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			_, err := os.Stat(filepath.Join(os.Getenv("CDX_CONFIG_DIR"), "best-relay"))
...
func TestApplyRememberedSendOptionsDisableClipboard(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			args:       []string{"cdx", "--disable-clipboard=false", "send"},
			remembered: true,
			want:       false,
		},
		{
			name:       "explicit true overrides remembered false",
			args:       []string{"cdx", "--disable-clipboard", "send"},
			remembered: false,
			want:       true,
		},
	}
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			for _, command := range app.Commands {
				...
				command.Action = func(ctx *cli.Context) error {
					...
					applyRememberedSendOptions(ctx, &got, transfer.Options{DisableClipboard: tt.remembered})
					return nil
				}
				if err := app.Run(tt.args); err != nil {
