...
func newApp() *cli.App {
	...
	app.UsageText = `cdx [GLOBAL OPTIONS] [COMMAND] [COMMAND OPTIONS] [filename(s) or folder]
   ...
   Send multiple files:
      cdx send file1.txt file2.txt file3.txt
    or
      cdx send *.jpg

   Send everything in a folder:
      cdx send example-folder-name

   Send a file with a custom code:
      cdx send --code secret-code file.txt
   ...
   Receive a file using code:
	...
	app.Commands = []*cli.Command{
		{
			Name:        "send",
			Usage:       "send file(s), or folder (see options with cdx send -h)",
			...
			Flags: []cli.Flag{
				...
				&cli.StringFlag{Name: "store-url", Value: "https://getcroc.com", Usage: "stored-transfer service origin", EnvVars: []string{"CDX_STORE_URL"}},
				&cli.StringFlag{Name: "mail", Value: "", Usage: "email the share code/link to this address (requires 'cdx login')"},
				&cli.BoolFlag{Name: "no-web", Usage: "do not start the built-in browser receive server"},
				&cli.StringFlag{Name: "web-url", Value: "", Usage: "base URL for browser receive links instead of the built-in server", EnvVars: []string{"CDX_WEB_URL"}},
			},
			HelpName: "cdx send",
			Action:   send,
		},
		{
			Name:        "serve",
			Usage:       "serve the browser receive page for share links",
			Description: "run the built-in web receiver so recipients can open share links in a browser",
			HelpName:    "cdx serve",
			Action:      serve,
			Flags: []cli.Flag{
				&cli.StringFlag{Name: "bind", Value: "0.0.0.0:9014", Usage: "local HTTP bind address"},
				&cli.StringFlag{Name: "relays", Aliases: []string{"relay"}, Value: "", Usage: "comma-separated upstream relay hosts (defaults to the public pool)"},
			},
		},
		{
			Name:        "login",
			Usage:       "save mail account credentials for 'cdx send --mail'",
			Description: "log in with an SMTP account so transfers can be shared by email",
			HelpName:    "cdx login",
			Action:      login,
			Flags: []cli.Flag{
				&cli.StringFlag{Name: "smtp-host", Value: "", Usage: "SMTP server host (for example smtp.gmail.com)"},
				&cli.IntFlag{Name: "smtp-port", Value: 587, Usage: "SMTP server port (587 STARTTLS, 465 TLS)"},
				&cli.StringFlag{Name: "user", Aliases: []string{"u"}, Value: "", Usage: "SMTP username"},
				&cli.StringFlag{Name: "pass", Aliases: []string{"p"}, Value: "", Usage: "SMTP password (prefer env CDX_SMTP_PASS)", EnvVars: []string{"CDX_SMTP_PASS"}},
				&cli.StringFlag{Name: "from", Value: "", Usage: "from address (defaults to username)"},
				&cli.BoolFlag{Name: "status", Usage: "show whether you are logged in"},
				&cli.BoolFlag{Name: "logout", Usage: "remove saved mail credentials"},
			},
		},
		{
			Name:        "relay",
			...
			Flags: []cli.Flag{
				...
				&cli.StringFlag{Name: "ports", Value: "9009,9010,9011,9012,9013", Usage: "ports of the relay", EnvVars: []string{"CDX_PORTS"}},
				&cli.IntFlag{Name: "port", Value: 9009, Usage: "base port for the relay", EnvVars: []string{"CDX_PORT"}},
				&cli.IntFlag{Name: "transfers", Value: 5, Usage: "number of ports to use for relay"},
				&cli.IntFlag{Name: "max-rooms-open", Value: tcp.DEFAULT_MAX_ROOMS_OPEN, Usage: "maximum waiting rooms per relay port", EnvVars: []string{"CDX_MAX_ROOMS_OPEN"}},
				&cli.IntFlag{Name: "max-pending-handshakes", Value: tcp.DEFAULT_MAX_PENDING_HANDSHAKES, Usage: "maximum incomplete handshakes per relay port", EnvVars: []string{"CDX_MAX_PENDING_HANDSHAKES"}},
				&cli.DurationFlag{Name: "handshake-timeout", Value: tcp.DEFAULT_HANDSHAKE_TIMEOUT, Usage: "maximum time for an initial relay handshake", EnvVars: []string{"CDX_HANDSHAKE_TIMEOUT"}},
				&cli.IntFlag{Name: "source-join-limit", Value: tcp.DEFAULT_SOURCE_JOIN_LIMIT, Usage: "maximum room joins per source IP in the admission window", EnvVars: []string{"CDX_SOURCE_JOIN_LIMIT"}},
				&cli.IntFlag{Name: "room-join-limit", Value: tcp.DEFAULT_ROOM_JOIN_LIMIT, Usage: "maximum joins per room in the admission window", EnvVars: []string{"CDX_ROOM_JOIN_LIMIT"}},
				&cli.DurationFlag{Name: "join-limit-window", Value: tcp.DEFAULT_JOIN_LIMIT_WINDOW, Usage: "sliding window for relay admission limits", EnvVars: []string{"CDX_JOIN_LIMIT_WINDOW"}},
			},
		},
		{
	...
	app.Flags = []cli.Flag{
		...
		&cli.StringFlag{Name: "relay", Value: models.DEFAULT_RELAY, Usage: "address of the relay", EnvVars: []string{"CDX_RELAY"}},
		&cli.StringFlag{Name: "relay6", Value: models.DEFAULT_RELAY6, Usage: "ipv6 address of the relay", EnvVars: []string{"CDX_RELAY6"}},
		&cli.StringFlag{Name: "out", Value: ".", Usage: "specify an output folder to receive the file"},
		&cli.StringFlag{Name: "pass", Value: models.DEFAULT_PASSPHRASE, Usage: "password for the relay", EnvVars: []string{"CDX_PASS"}},
		&cli.StringFlag{Name: "socks5", Value: "", Usage: "add a socks5 proxy", EnvVars: []string{"SOCKS5_PROXY"}},
		&cli.StringFlag{Name: "connect", Value: "", Usage: "add a http proxy", EnvVars: []string{"HTTP_PROXY"}},
...
Do you wish to continue to DISABLE the classic mode? (y/N) `, colorEnabled))
				...
				if choice == "y" || choice == "yes" {
					...
					fmt.Print(`To send and receive, export the CDX_SECRET variable with the code phrase:

  Send:    CDX_SECRET=*** cdx send file.txt

  Receive: CDX_SECRET=*** cdx` + "\n\n")
				} else {
...
Do you wish to continue to enable the classic mode? (y/N) `, colorEnabled))
  ...
  Receive: cdx ***` + "\n\n")
		...
		if c.Args().Present() && allStringsAreFiles(c.Args().Slice()) {
			...
			if errInput != nil {
				return fmt.Errorf("could not read confirmation (use 'cdx send' to send without one): %w", errInput)
			}
			choice = strings.ToLower(choice)
...
func usesPublicRelay(c *cli.Context, options transfer.Options) bool {
...
func assignPublicRelay(options *transfer.Options, relayIndex int) error {
...
func assignPublicRelayForCode(options *transfer.Options) error {
	relays := publicrelay.Relays()
...
func clearBestPublicRelayOnSendError(generatedPublicCode bool, err error) {
	if generatedPublicCode && errors.Is(err, transfer.ErrRelayConnection) {
...
func applyRememberedSendOptions(c *cli.Context, options *transfer.Options, remembered transfer.Options) {
...
func send(c *cli.Context) (err error) {
	...
	if transferOpts.RelayAddress != models.DEFAULT_RELAY {
		transferOpts.RelayAddress6 = ""
	} else if transferOpts.RelayAddress6 != models.DEFAULT_RELAY6 {
		transferOpts.RelayAddress = ""
	}
	b, errOpen := os.ReadFile(getSendConfigFile(false))
	if errOpen == nil && !c.Bool("remember") {
		var rememberedOptions transfer.Options
		err = json.Unmarshal(b, &rememberedOptions)
		if err != nil {
			log.Error(err)
			return
		}
		applyRememberedSendOptions(c, &transferOpts, rememberedOptions)
	}
	publicRelayMode := usesPublicRelay(c, transferOpts)
	...
	if len(fnames) == 0 {
		return errors.New("must specify file: cdx send [filename(s) or folder]")
	}

	classicInsecureMode := utils.Exists(getClassicConfigFile(true))
	envSecret := os.Getenv("CDX_SECRET")
	transferOpts.SharedSecret = resolveSendSharedSecret(transferOpts.SharedSecret, envSecret)
...
you need to set the environmental variable CDX_SECRET:

  CDX_SECRET=**** cdx send file.txt

Or you can have the code phrase automatically generated:

  cdx send file.txt

Or you can go back to the classic cdx behavior by enabling classic mode:
...
`)
	...
	generatedPublicCode := len(transferOpts.SharedSecret) == 0 && publicRelayMode
	var relayIndex int
	if len(transferOpts.SharedSecret) == 0 {
		if publicRelayMode {
			relayIndex, err = selectPublicRelay(tcp.MeasureServerLatencyContext)
			if err != nil {
				// No public relay reachable: fall back to a local relay so the
				// transfer still works over LAN/direct connections.
				log.Warnf("no public relay available (%v); falling back to a local relay", err)
				publicRelayMode = false
				generatedPublicCode = false
			}
		}
		if publicRelayMode {
			transferOpts.SharedSecret, err = codephrase.GenerateForRelay(relayIndex, len(publicrelay.Relays()))
			if err == nil {
				err = assignPublicRelay(&transferOpts, relayIndex)
			}
		} else {
			transferOpts.SharedSecret, err = codephrase.Generate()
		}
		if err != nil {
			return fmt.Errorf("could not generate code phrase: %w", err)
		}
	} else if publicRelayMode {
		if err = assignPublicRelayForCode(&transferOpts); err != nil {
			log.Warnf("no public relay available (%v); falling back to a local relay", err)
			publicRelayMode = false
			generatedPublicCode = false
		}
	}
	// CD does not operate hosted relays: when no explicit relay was given,
	// use local-network/direct mode instead of dialing unreachable defaults.
	if !publicRelayMode && transferOpts.RelayAddress == models.DEFAULT_RELAY && transferOpts.RelayAddress6 == models.DEFAULT_RELAY6 {
		transferOpts.RelayAddress = ""
		transferOpts.RelayAddress6 = ""
		transferOpts.OnlyLocal = true
	}
	if err = emailShareCode(c, transferOpts.SharedSecret); err != nil {
		return
	}
	webBase, stopWeb := resolveWebBaseURL(c.String("web-url"), !c.Bool("no-web") && !transferOpts.DisableLocal, portParam, transfersParam)
	defer stopWeb()
	transferOpts.WebURL = webBase
	minimalFileInfos, emptyFoldersToTransfer, totalNumberFolders, err := transfer.GetFilesInfoWithExactExclusions(fnames, transferOpts.ZipFolder, transferOpts.GitIgnore, transferOpts.Exclude, transferOpts.ExcludeFile)
	if err != nil {
		return
	}
	if len(transferOpts.Exclude) > 0 {
		minimalFileInfosInclude := []transfer.FileInfo{}
		emptyFoldersToTransferInclude := []transfer.FileInfo{}
		for _, f := range minimalFileInfos {
			exclude := false
			for _, exclusion := range transferOpts.Exclude {
	...
	cr, err := transfer.New(transferOpts)
	if err != nil {
		return
	}

	// save the config
	saveConfig(c, transferOpts)
...
// files, so it does not harden configs created by older cdx versions.
...
func saveConfig(c *cli.Context, transferOpts transfer.Options) {
	if c.Bool("remember") {
		...
		if c.String("code") == "" {
			transferOpts.SharedSecret = ""
		}
		if c.String("relay") != models.DEFAULT_RELAY {
			transferOpts.RelayAddress = "non-default: " + c.String("relay")
		} else {
			transferOpts.RelayAddress = "default"
		}
		if c.String("relay6") != models.DEFAULT_RELAY6 {
			transferOpts.RelayAddress6 = "non-default: " + c.String("relay6")
		} else {
			transferOpts.RelayAddress6 = "default"
		}
		bConfig, err := json.MarshalIndent(transferOpts, "", "    ")
		if err != nil {
			log.Error(err)
			return
		}
...
func receive(c *cli.Context) (err error) {
	...
	comm.Socks5Proxy = c.String("socks5")
	comm.HttpProxy = c.String("connect")
	if storedToken := strings.TrimSpace(os.Getenv("CDX_STORE_TOKEN")); storedToken != "" {
		setDebugLevel(c)
	...
	switch c.Args().Len() {
	case 1:
		transferOpts.SharedSecret = c.Args().First()
	case 3:
		fallthrough
	case 4:
		var phrase []string
		...
		transferOpts.SharedSecret = strings.Join(phrase, "-")
	}
	if storeclient.IsStoredValue(transferOpts.SharedSecret) {
		setDebugLevel(c)
...
Run cdx with no argument and paste the link at the prompt, or use:

  CDX_STORE_TOKEN='cdx-store-v1....' cdx

`)
			return nil
		}
		return receiveStored(c, transferOpts.SharedSecret)
	}
	...
	if errOpen == nil && !doRemember {
		...
		if !c.IsSet("yes") {
			transferOpts.NoPrompt = rememberedOptions.NoPrompt
		}
		if transferOpts.SharedSecret == "" {
			transferOpts.SharedSecret = rememberedOptions.SharedSecret
		}
		if !c.IsSet("pass") && rememberedOptions.RelayPassword != "" {
			transferOpts.RelayPassword = rememberedOptions.RelayPassword
		}
		if !c.IsSet("overwrite") {
			transferOpts.Overwrite = rememberedOptions.Overwrite
		}
		if !c.IsSet("rename") {
			transferOpts.Rename = rememberedOptions.Rename
		}
		if !c.IsSet("curve") && rememberedOptions.Curve != "" {
			transferOpts.Curve = rememberedOptions.Curve
		}
		if !c.IsSet("local") {
			transferOpts.OnlyLocal = rememberedOptions.OnlyLocal
		}
		if !c.IsSet("relay") && strings.HasPrefix(rememberedOptions.RelayAddress, "non-default:") {
			...
			transferOpts.RelayAddress = rememberedAddr
		}
		if !c.IsSet("relay6") && strings.HasPrefix(rememberedOptions.RelayAddress6, "non-default:") {
			...
			rememberedAddr = strings.TrimSpace(rememberedAddr)
			transferOpts.RelayAddress6 = rememberedAddr
		}
	}
	publicRelayMode := usesPublicRelay(c, transferOpts)

	classicInsecureMode := utils.Exists(getClassicConfigFile(true))
	if transferOpts.SharedSecret == "" && os.Getenv("CDX_SECRET") != "" {
		transferOpts.SharedSecret = os.Getenv("CDX_SECRET")
	} else if !(runtime.GOOS == "windows") && transferOpts.SharedSecret != "" && !classicInsecureMode {
		commandLineSecret := transferOpts.SharedSecret
		transferOpts.SharedSecret = os.Getenv("CDX_SECRET")
		if transferOpts.SharedSecret == "" {
			output, colorEnabled := termui.Output(os.Stdout)
	...
	if transferOpts.SharedSecret == "" {
		transferOpts.SharedSecret, err = utils.GetInput("Enter receive code: ")
		if err != nil {
	...
	if publicRelayMode {
		if err = assignPublicRelayForCode(&transferOpts); err != nil {
			log.Warnf("no public relay available (%v); using local network discovery", err)
			publicRelayMode = false
		}
	}
	// Mirror the sender's LAN-first default: without an explicit relay,
	// discover the sender on the local network instead of dialing a
	// hosted relay that CD does not operate.
	if !publicRelayMode && !c.IsSet("relay") && transferOpts.RelayAddress == models.DEFAULT_RELAY && transferOpts.RelayAddress6 == models.DEFAULT_RELAY6 {
		transferOpts.RelayAddress = ""
		transferOpts.RelayAddress6 = ""
		transferOpts.OnlyLocal = true
	}
...
func relay(c *cli.Context) (err error) {
	log.Infof("starting cdx relay version %v", Version)
