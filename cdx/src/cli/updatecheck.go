...
const (
	latestReleaseEndpoint  = "https://api.github.com/repos/schollz/cdx/releases/latest"
...
func (checker transferVersionChecker) fetchLatest(ctx context.Context) (string, error) {
	...
	request.Header.Set("User-Agent", "cdx/"+checker.currentVersion)
...
func forceTransferVersionCheck() bool {
	return os.Getenv("CDX_DO_CHECK") == "1"
}
...
func writeTransferVersionNotice(c *internalcli.Context, checker transferVersionChecker, result versionCheckResult, always bool) {
	...
	if result.latestVersion == "" {
		_, _ = fmt.Fprintf(writer, "Could not check for a newer cdx version (current: v%s).\n", checker.currentVersion)
		return
	}
	_, _ = fmt.Fprintf(
		writer,
		"No newer cdx version is available (current: v%s, latest: v%s).\n",
		checker.currentVersion,
		result.latestVersion,
	)
}
