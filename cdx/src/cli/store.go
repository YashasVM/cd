...
func sendStored(c *cli.Context) error {
	...
	paths := c.Args().Slice()
	if len(paths) == 0 {
		return errors.New("must specify file: cdx send --store [filename(s)]")
	}
	...
	if !c.Bool("disable-clipboard") {
		transfer.CopyToClipboard(browserURL, c.Bool("quiet"), false)
	}
	if c.Bool("qrcode") {
		transfer.ShowReceiveCommandQrCode(browserURL)
	}
	return nil
}
...
func revokeStored(c *cli.Context, transferID string) error {
	...
	if id == "" || c.Args().Present() {
		return errors.New("usage: cdx --revoke [transfer-id]")
	}
	receipts, err := readStoreReceipts()
	if err != nil {
		return err
