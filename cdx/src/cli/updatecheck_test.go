	...
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			server := httptest.NewServer(http.HandlerFunc(func(response http.ResponseWriter, request *http.Request) {
				...
				if got := request.Header.Get("User-Agent"); got != "cdx/11.2.2" {
...
func TestCDXDoCheckRequiresExactValue(t *testing.T) {
	for _, test := range []struct {
		value string
		want  bool
	}{
		...
		t.Run(test.value, func(t *testing.T) {
			t.Setenv("CDX_DO_CHECK", test.value)
...
func TestTransferVersionNoticeIsDeferredAndHonorsQuiet(t *testing.T) {
	...
	want := fmt.Sprintf(
		"A newer cdx version is available: v%s (current: v%s).\nRun: curl https://getcroc.com | bash\n",
		latest,
		Version,
	)
...
func TestForcedTransferVersionCheckReportsBeforeTransfer(t *testing.T) {
	for _, test := range []struct {
	...
	}{
		{
			...
			want:       "No newer cdx version is available (current: v11.2.3, latest: v11.2.3).\n",
		},
		{
			name:       "newer",
			statusCode: http.StatusOK,
			body:       `{"tag_name":"v11.2.4"}`,
			want:       "A newer cdx version is available: v11.2.4 (current: v11.2.3).\nRun: curl https://getcroc.com | bash\n",
		},
		{
			name:       "unavailable",
			statusCode: http.StatusServiceUnavailable,
			body:       `{}`,
			want:       "Could not check for a newer cdx version (current: v11.2.3).\n",
		},
		{
			name:       "quiet",
			statusCode: http.StatusOK,
...
func TestFailedSendAndReceiveRunDeferredVersionNotice(t *testing.T) {
	...
	for _, test := range []struct {
		...
		{name: "send", args: []string{"cdx", "--ignore-stdin", "send"}},
		{name: "stored send", args: []string{"cdx", "--ignore-stdin", "send", "--store"}},
		{name: "stored receive", args: []string{"cdx", "--ignore-stdin"}, storedToken: "invalid-stored-token"},
	} {
		t.Run(test.name, func(t *testing.T) {
			configDir := t.TempDir()
			t.Setenv("CDX_CONFIG_DIR", configDir)
			t.Setenv("CDX_DO_CHECK", "0")
			t.Setenv("CDX_STORE_TOKEN", test.storedToken)
			if err := writeVersionCheckCache(filepath.Join(configDir, versionCheckCacheName), versionCheckCache{
				CheckedAt:     time.Now(),
				LatestVersion: latest,
			}); err != nil {
