...
func TestStoredCallbacksUseRegularCdxPalette(t *testing.T) {
	var output bytes.Buffer
...
func TestFormatStoredSendInstructionsUsesRegularCdxPalette(t *testing.T) {
	plain := formatStoredSendInstructions(
		"tomorrow", "https://example.com/#secret", "cdx-store-v1.token", "transfer-id", "one verified download", false,
	)
	colored := formatStoredSendInstructions(
		"tomorrow", "https://example.com/#secret", "cdx-store-v1.token", "transfer-id", "one verified download", true,
	)
	if termui.Plain(colored) != plain {
		t.Fatalf("colored stored instructions changed text:\n%s", colored)
	}
	for _, secret := range []string{"https://example.com/#secret", "cdx-store-v1.token", "transfer-id"} {
